...
```{r}
#INT<-sapply(1:11,function(x){which(asia$INT==x)})
nodes<-names(asia)
#names(INT)<-nodes
...
netlist<-lapply(start_asia,function(net){tabu(asia,score="mbde",iss=1,start=net,tabu=50)})
...
compare((dag.asia_groundtruth),(dag.mbde_asia),  arcs = TRUE)
...
netlist_prior<-lapply(start_asia,function(net){tabu(asia,score="mbde", prior = "cs",beta=beta,iss=1,start=net,tabu=50)})

netlist_prior_marginal<-lapply(start_asia,function(net){tabu(asia,score="mbde", prior = "marginal",iss=1,start=net,tabu=50)})

netlist_prior_vsp<-lapply(start_asia,function(net){tabu(asia,score="mbde", prior = "vsp",iss=1,start=net,tabu=50)})
